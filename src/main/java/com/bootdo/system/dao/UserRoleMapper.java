...
public interface UserRoleMapper {
    ...
    @Insert("<script>" + "INSERT INTO sys_user_role(user_id, role_id) values" + " <foreach collection=\"list\" item=\"item\" index=\"index\" separator=\",\" >  " + " (#{item.userId},#{item.roleId})"
            + " </foreach>" + "</script>")
    ...
    @Delete("<script>" + "delete from sys_user_role where user_id in" + " <foreach collection=\"list\" index=\"i\" open=\"(\" separator=\",\" close=\")\" item=\"item\"  >#{item}</foreach>"
            + "</script>")
